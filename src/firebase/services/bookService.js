...
export const updateBookByUserIdAndTitle = async (data, title) => {
  const currentBookData = await getBookByTitleAndUserId(title);
  const updatedData = { ...currentBookData, ...data };
  const userId = JSON.parse(storage.getItem("user")).email;
  try {
    // Reference to the books collection
    const bookCollection = collection(db, "books");
    ...
    if (!querySnapshot.empty) {
      ...
      console.log(
        "Book updated successfully with data" + JSON.stringify(updatedData)
      );
      return updatedData;
    } else {
