...
export default function ImageUpload({
  ...
  const handleUpload = async (imageSrc) => {
    ...
    await Promise.all([
      getPageSummaryFromImageStream(croppedFile, 5, (chunk) => {
        ...
        scanDataResponse = [
          {
            ...scanDataResponse[0],
            summary: scanDataResponse[0].summary + chunk,
          },
        ];
        setData((prev) => {
          return prev
            ? {
                ...prev,
                data: [
                  { ...prev.data[0], summary: prev.data[0].summary + chunk },
                ],
              }
            : {
                bookTitle: bookTitle,
                data: [{ simpleLang: "", summary: chunk }],
              };
        });
      }),
      getSimplifiedLanguageStream(croppedFile, (chunk) => {
        ...
        scanDataResponse = [
          {
            ...scanDataResponse[0],
            simpleLang: scanDataResponse[0].simpleLang + chunk,
          },
        ];
        setData((prev) => {
          return prev
            ? {
                ...
                data: [
                  {
                    ...prev.data[0],
                    simpleLang: prev.data[0].simpleLang + chunk,
                  },
                ],
              }
            : {
                bookTitle: bookTitle,
                data: [{ summary: "", simpleLang: chunk }],
              };
        });
      }),
    ]);
    ...
    const timeDifferenceInSeconds =
      (Date.now() - profile?.streak.lastPageScanTimestamp) / 1000;
    // alert(timeDifferenceInSeconds);
    if (
      timeDifferenceInSeconds > streakMaintenanceIntervalInSeconds &&
      timeDifferenceInSeconds < streakMaintenanceIntervalInSeconds * 2
    ) {
      // < 48 hours & > 24 hours
      setProfile(
      ...
      streakMaintenanceIntervalInSeconds * 2
    ) {
      setProfile(
        await updateProfile(profile?.userId, {
          ...profile,
        ...
        <span>
          ...
          <div className={styles.fileInputContainer}>
            ...
            <label htmlFor="file-upload" className={styles.fileInputLabel}>
              {uploadingImage ? (
                <Loader size={32} color="white" className="loader" />
              ) : (
                <Camera
                  onClick={() => {
                    logGAEvent("click_scan_first_page_of_book");
                  }}
                  size={32}
                  color="white"
                />
